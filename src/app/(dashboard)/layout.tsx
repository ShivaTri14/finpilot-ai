        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-8">
            <Link href="/dashboard" className="flex items-center gap-2 group">
              <div className="h-9 w-9 rounded-xl bg-gradient-to-tr from-emerald-500 to-cyan-400 flex items-center justify-center shadow-lg shadow-emerald-500/20 group-hover:scale-105 transition-transform">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden lg:flex items-center gap-2 text-xs bg-slate-900/80 border border-slate-800 rounded-lg px-3 py-1.5 text-slate-400">
              <span className="font-semibold text-white">{userSymbol} {userCurrency}</span>
            ...
            >
              <div className="h-6 w-6 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-xs font-semibold text-emerald-400">
                {session?.user?.name ? session.user.name[0].toUpperCase() : "U"}
              </div>
              <span className="font-medium text-xs hidden sm:inline">{session?.user?.name || "User"}</span>
            </Link>
